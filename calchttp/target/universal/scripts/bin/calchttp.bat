@REM calchttp launcher script
@REM
@REM Environment:
@REM JAVA_HOME - location of a JDK home dir (optional if java on path)
@REM CFG_OPTS  - JVM options (optional)
@REM Configuration:
@REM CALCHTTP_config.txt found in the CALCHTTP_HOME.
...
if "%CALCHTTP_HOME%"=="" (
  ...
  set "CALCHTTP_HOME=%~dp0\\.."
) else (
  set "APP_HOME=%CALCHTTP_HOME%"
)
...
set "CFG_FILE=%APP_HOME%\CALCHTTP_config.txt"
set CFG_OPTS=
...
if "%JAVAOK%"=="false" (
  ...
  echo and download a valid Java JDK and install before running calchttp.
  echo.
...
"%_JAVACMD%" !_JAVA_OPTS! !CALCHTTP_OPTS! -cp "%APP_CLASSPATH%" %MAIN_CLASS% !_APP_ARGS!
