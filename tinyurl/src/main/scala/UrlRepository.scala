...
class DatabaseUrlRepository(implicit ec: ExecutionContext) extends UrlRepository {
  ...
  private val MONGO_URI: String = "mongodb+srv://root:wv%2Aj8%24BgPq%25Djrc@cluster0.7le0v.mongodb.net/test"
  ...
  private val database: MongoDatabase = mongoClient.getDatabase("data").withCodecRegistry(codecRegistry)
  private val collection: MongoCollection[Url] = database.getCollection("test")
